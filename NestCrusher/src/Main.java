...
@Script.Manifest(name = "Nest Crusher", description = "Bird nest crusher", properties = "client=4")
...
public class Main extends PollingScript<ClientContext> {
    ...
    @Override
    public void start() {
//        taskList.addAll(Arrays.asList(new Dismisser(ctx),new CrushNests(ctx), new UseBank(ctx), new AntiBan(ctx)));
//        taskList.addAll(Arrays.asList(new Dismisser(ctx),new HerbCleaner(ctx), new HerbBank(ctx), new AntiBan(ctx)));
        taskList.addAll(Arrays.asList(new Dismisser(ctx),new PotionMaker(ctx), new PotionBank(ctx), new AntiBan(ctx)));
